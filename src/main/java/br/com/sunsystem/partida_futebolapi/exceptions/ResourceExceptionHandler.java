...
public class ResourceExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<StandardError> badRequestException(
			BadRequestException bad, HttpServletRequest request){
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new StandardError(HttpStatus.BAD_REQUEST, 
				bad.getMessage(), request.getRequestURI()));
	}
	
	@ExceptionHandler(UnauthorizedException.class)
	public ResponseEntity<StandardError> unauthorizedException(
			UnauthorizedException e, HttpServletRequest request){
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new StandardError(HttpStatus.UNAUTHORIZED, 
				e.getMessage(), request.getRequestURI()));
	}
	
	@ExceptionHandler(ForbbidenException.class)
	public ResponseEntity<StandardError> forbbidenException(
			ForbbidenException e, HttpServletRequest request){
	...
	public ResponseEntity<StandardError> runTimeException(
